...
function clearMealPlan() {
    ...
    db.collection('mealPlan').doc('current').set(mealPlan).then(() => {
        ...
        // Используем только иконку, без текста, чтобы на маленьких экранах не было проблем
        ...
        console.error("Erreur lors de l'effacement du plan:", error);
    });
}

// Новая улучшенная функция для отправки списка покупок в Telegram
// Функция настройки списка покупок
function setupShoppingList() {
    // Обработчик для печати списка покупок
    document.getElementById('print-shopping-list-btn').addEventListener('click', function() {
    ...
    // Новый обработчик для отправки списка в Telegram
    document.getElementById('share-telegram-btn').addEventListener('click', shareShoppingListToTelegram);
    
    // Обработчик для чекбоксов списка покупок
    document.addEventListener('change', function(e) {
        // Существующий код обработки чекбоксов...
    });
}

// Новая функция для отправки списка покупок в Telegram
// Добавьте этот обработчик в функцию setupShoppingList
document.getElementById('share-telegram-btn').addEventListener('click', function() {
    shareShoppingListToTelegram();
});

// Новая упрощенная функция для отправки в Telegram
// Функция для отправки списка покупок в приложение Telegram
// Функция для отправки списка покупок в Telegram
function shareShoppingListToTelegram() {
    console.log("Функция shareShoppingListToTelegram запущена");
    
    // Создаем простой текст для списка покупок
    let shoppingListText = "Liste d'achats:\n\n";
    let totalPrice = 0;
    
    // Собираем все видимые категории и элементы
    const categories = {};
    
    // Находим все заголовки категорий и соответствующие элементы
    let currentCategory = "Divers";
    
    document.querySelectorAll('#shopping-list .list-group-item').forEach(item => {
        // Определяем, является ли элемент заголовком категории
        if (item.classList.contains('list-group-item-secondary')) {
        ...
        // Получаем текст продукта
        const label = item.querySelector('.form-check-label');
        if (!label) return;
        
        // Извлекаем чистый текст, удаляя HTML-теги
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = label.innerHTML;
        const productText = tempDiv.textContent.trim();
        
        // Добавляем в текущую категорию
        if (!categories[currentCategory]) {
            categories[currentCategory] = [];
        }
        
        categories[currentCategory].push(productText);
    ...
    // Формируем простой текст для Telegram
    ...
    if (isMobile) {
        // Для мобильных устройств используем прямой протокол tg:// для открытия приложения
        console.log("Мобильное устройство обнаружено, открываем приложение Telegram");
        
        // Создаем ссылку для открытия мобильного приложения
        const telegramAppUrl = `tg://msg?text=${encodedText}`;
        
        // Пытаемся открыть приложение через прямой протокол
        window.location.href = telegramAppUrl;
        
        // Проверяем, открылось ли приложение (через таймаут)
        setTimeout(function() {
            // Если мы все еще здесь, возможно, приложение не установлено
            // Предлагаем альтернативный вариант открытия
            if (confirm("Si l'application ne s'ouvre pas, voulez-vous essayer via le navigateur?")) {
                window.open(`https://t.me/share/url?url=&text=${encodedText}`, "_blank");
            }
        }, 1500);
    } else {
        // Для настольных компьютеров открываем веб-клиент K
        console.log("Настольный компьютер обнаружен, открываем веб-версию Telegram K");
        
        // Открываем новую вкладку с веб-клиентом Telegram K
        const telegramWebWindow = window.open("https://web.telegram.org/k/", "_blank");
        
        // Проверяем, не заблокировано ли открытие нового окна
        if (!telegramWebWindow || telegramWebWindow.closed || typeof telegramWebWindow.closed === 'undefined') {
            // Окно не открылось (возможно, блокировка всплывающих окон)
            alert("Veuillez autoriser les fenêtres pop-up pour ouvrir Telegram Web.");
        } else {
            // Копируем текст в буфер обмена для вставки в веб-клиент
            navigator.clipboard.writeText(shoppingListText).then(function() {
                alert("La liste a été copiée dans le presse-papiers. Vous pouvez la coller dans Telegram Web.");
            }).catch(function() {
                alert("Impossible de copier automatiquement. Veuillez copier manuellement votre liste dans Telegram Web.");
            });
        }
    }
    
    // Визуальное подтверждение
    const button = document.getElementById('share-telegram-btn');
    const originalText = button.innerHTML;
    button.innerHTML = '<i class="fab fa-telegram"></i> Envoyé';
...
// Добавление обработчика события для кнопки Telegram
document.addEventListener('DOMContentLoaded', function() {
    // Проверяем, существует ли кнопка
    const telegramButton = document.getElementById('share-telegram-btn');
    if (telegramButton) {
        console.log("Кнопка Telegram найдена, добавляем обработчик");
        telegramButton.addEventListener('click', shareShoppingListToTelegram);
    } else {
        console.error("Кнопка Telegram не найдена!");
        
        // Попытка найти контейнер для кнопок и добавить нашу кнопку
        const buttonsContainer = document.querySelector('.d-flex.justify-content-between.mb-3 div');
        if (buttonsContainer) {
            console.log("Контейнер для кнопок найден, добавляем кнопку Telegram");
            
            // Создаем кнопку
            const newTelegramButton = document.createElement('button');
            newTelegramButton.id = 'share-telegram-btn';
            newTelegramButton.className = 'btn btn-outline-primary';
            newTelegramButton.innerHTML = '<i class="fab fa-telegram"></i> Telegram';
            
            // Добавляем кнопку в контейнер
            buttonsContainer.appendChild(newTelegramButton);
            
            // Добавляем обработчик
            newTelegramButton.addEventListener('click', shareShoppingListToTelegram);
        }
    }
});
